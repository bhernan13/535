...
inputs{11,1} = 'Mass Flow [lbm/s]'; inputs{11,2} = mdot;
...
T_fluid(j,:) = inletTemp;
T_peb(:,i) = Setpoint;
% m_slug(:,i) = mdot{:,2}*dt; - moved below
...
% end
    ...
    % define mdot for this loop iteration
    if L>1
        if aa+1 <= L && mdot{aa+1,1} < t(i)
            aa = aa+1;
            if aa > L
                aa = aa-1;
            end
        end
        if isnumeric(mdot{aa,2}) % constant values of mdot
            m_slug_loop(i) = mdot{aa,2}*dt;
            mdot_loop(i) = mdot{aa,2};
        else % mdot is function of t
            m_slug_loop(i) = mdot{aa,2}(t(i))*dt;
            mdot_loop(i) = mdot{aa,2}(t(i));
        end
    else
        if isnumeric(mdot) % constant value of mdot
            m_slug_loop(i) = mdot*dt;
            mdot_loop(i) = mdot;
        else % mdot is function of t
            m_slug_loop(i) = mdot(t(i))*dt;
            mdot_loop(i) = mdot(t(i));
        end
    end
...
%        end
       ...
       V(j,i) = mdot_loop(i) / (rho(j,i)*xsectionArea);
       ...
       T_fluid(j+1,i) = T_fluid(j,i) + ( (H(j,i)*dt*areaDepth) / (m_slug_loop(i) * Cp_fluid(j,i)) ) * (T_peb(j,i) - T_fluid(j,i));
...
%        disp('Hi, Bug here, not leaving.');
       ...
       dP(j,i) = (pressuredrop(mdot_loop(i),porosity,bedID,pebbleDiameter,dt,V(j,i),rho(j,i),mu(j,i),N))*conv.psi2pa;
       % Pressure 
...
if isempty(outputs)
    ...
    if any(contains(outputs(:,1),'dens','IgnoreCase',true)) 
        Outputs{indexOut,1} = 'Density [kg/m^3]';
        Outputs{indexOut,2} = rho;
        indexOut = indexOut+1;
    end
    if any(contains(outputs(:,1),'reyn','IgnoreCase',true)) 
        Outputs{indexOut,1} = 'Reynolds number';
        Outputs{indexOut,2} = Re1;
        indexOut = indexOut+1;
    end
    if any(contains(outputs(:,1),'pran','IgnoreCase',true)) 
        Outputs{indexOut,1} = 'Prandtl number';
        Outputs{indexOut,2} = Pr;
        indexOut = indexOut+1;
    end
    if any(contains(outputs(:,1),'mass','IgnoreCase',true)) 
        Outputs{indexOut,1} = 'Mass Flow [kg/s]';
        Outputs{indexOut,2} = mdot_loop;
        indexOut = indexOut+1;
    end
end
